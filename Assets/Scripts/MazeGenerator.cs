...
public class MazeGenerator : MonoBehaviour
{
    ...
    public void RestartMaze(int nWidth, int nHeight)
    {
        ...
        for (int i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (int j = 0; j <= Maze.GetUpperBound(1); j++)
            {
                if (Maze[i, j] == 1)
                {
                    ...
                    ///MODIFY
                    ptype.transform.localScale = Vector3.one * 10;
                    ///modified y to be z axis instead
                ...
                {
                    ///modify y axis to be z axis instead
                    pathMazes.Add(new Vector3(i, 0, j));

                    //MODIFY added
                    if ((EndTile == null) || !(EndTile[0] == i && EndTile[1] == j))
                    {
                        GameObject go = Instantiate(Resources.Load(TileUtilities.open)) as GameObject;
                        go.transform.position = new Vector3(i * 10, 0, j * 10);
                        go.transform.parent = this.transform;
                        if (isStart)
                        {
                            go.transform.GetChild(1).GetComponent<Renderer>().material.color = Color.green;
                            isStart = false;
                        }
                    ...
                    {
                        GameObject go = Instantiate(Resources.Load(TileUtilities.levelEnd)) as GameObject;
                        go.transform.position = new Vector3(i * 10, 0, j * 10);
                        go.transform.parent = this.transform;
                    }
                    //end MODIFY
    ...
    public int[,] CreateMaze()
    {
        ...
        while (_tiletoTry.Count > 0)
        {
            ...
            else
            {
                ...
                {
                    EndTile[0] = (int)CurrentTile.x;
                    EndTile[1] = (int)CurrentTile.y;
